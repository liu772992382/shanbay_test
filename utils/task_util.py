...
def get_tasks_user(uid):
    ...
    try:
        tmp_tasks = session.query(Task).filter_by(uid=uid).all()
        for i in tmp_tasks:
            tmp_task = i.get_dict()
            tmp_task['word'] = session.query(Word).filter_by(wid=i.wid).first().get_dict()
...
def create_tasks(uid, bid):
    tmp = {'status': False}
    try:
        tmp_words = session.query(WordBook).filter_by(bid=bid).all()
        for i in tmp_words:
